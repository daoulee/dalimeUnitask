...
---
1. AI 에게 설계를 맡기는 것은 매우 부적절하고 . 소속된 조직에 위험이 될 수 있음 .
2. Claude 가 아키텍트가 될 수 없는 4가지 이유 . 
  -1. 패턴 매칭의 한계 
  -2. 거절 능력 부재
  -3. 현실 제약 무시 
  -4. 책임 공백
  **좋은 아키텍트는 "하지맙시다" 라고 말한다.**

  AI에 의존할 경우 **결국 "젠가 탑 같은" 아키텍쳐가 나온다.
  
  **인간이 맥락과 트레이드 오프를 바탕으로 설계하고, AI가 그 설계를 빠르게 구현하도록 보조한다.**
  ㄴ 이떄에 Trade Off의 뜻은 장점을 얻으면 단점을 감수해야 하는 상황이다.

  AI한테 설계를 물어보되 , 검토와 수정 . 보완을 내가 해야한다. 
  "지금 이 코드가 내 애플리케이션의 전체 맥락과 맞는지 , 
  다른 부분과 충돌하지는 않는지 , 
  보안상 안전한지 , 
  장기적으로 유지보수하기 좋은 구조인지 

  **ADR 이란 ?**
  아키텍쳐 결정 사항과 그 이유를 짧은 문서로 남기는 기록 ,
  무엇을 채택했는지 뿐만 아니라 무엇을 의도적으로 거절했는지 함께 적어둔다.
